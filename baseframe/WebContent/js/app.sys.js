						...
						handler : function(){
							...
							function checkInfo() {
								...
								$.get(base+'/sys/file/info',{},function(data){
									...
									var info = '文件大小：' + Math.round(data.sum/1000) + 'KB　　已上传：' + Math.round(data.current/1000) + 'KB　　速度：'  + Math.round(data.current/(nowTime-startTime)) + 'K/s';
...
if(!WX.sys.enable) WX.sys.enable = {};
...
if(!WX.sys.gender) WX.sys.gender = {};
...
if(!WX.sys.station) WX.sys.station = {};
